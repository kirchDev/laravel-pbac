...
- The order in `PbacServiceProvider::MIGRATIONS` is **load-bearing**, not cosmetic. Both pivot tables carry foreign keys to `roles`/`permissions`, and each entry is published one second after the previous one so the consumer's migrator sorts them that way. Give every new migration its place in that list; publishing two under the same timestamp puts the migrator back on alphabetical order, which runs `create_model_has_roles_table` first and breaks the foreign key.
- The **source** filenames in `database/migrations/` keep their `2026_05_09_*` prefixes purely as an internal sort key — the test suite runs them with `migrate --path`. Consumers never see those names.
